...
class BertForBasicNER(PreTrainedBertModel):
    ...
    def forward(self, input_ids, input_masks,
        ...
        for i in range(0, input_ids.size(0), self.batch_size):
            tmp_input_ids = input_ids[i: i + self.batch_size]
            tmp_input_masks = input_ids[i: i + self.batch_size]
            tmp_batch_seq_enc, _ = self.bert(tmp_input_ids,
                                             attention_mask=tmp_input_masks,
                                             output_all_encoded_layers=False)
            batch_seq_enc.append(tmp_batch_seq_enc)
...
class NERModel(nn.Module):
    ...
    def forward(self, input_ids, input_masks,
        ...
        batch_seq_enc = []
        for i in range(0, input_ids.size(0), self.batch_size):
            tmp_input_emb = input_emb[i: i + self.batch_size]
            tmp_input_masks = input_masks[i: i + self.batch_size]
            tmp_batch_seq_enc = self.token_encoder(tmp_input_emb, tmp_input_masks)
            batch_seq_enc.append(tmp_batch_seq_enc)
        batch_seq_enc = torch.cat(batch_seq_enc, dim=0)
