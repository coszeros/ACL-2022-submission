...
from .utils import BERTChineseCharacterTokenizer, default_dump_json, default_load_pkl, save_json
...
from .dee_model import CABModel
...
class DEETaskSetting(TaskSetting):
    ...
    base_attr_default_pairs = [
        ...
        ('cpt_file_name', 'CAB'),  # decide the identity of checkpoints, evaluation results, etc.
        ('model_type', 'CAB'),  # decide the model class used
        ...
        # ------------------ CAB setting -------------------
        ('skip_ed', False),  # remove event detection
        ('hybrid_extract', True),  # using hybrid extract
    ]
...
class DEETask(BasePytorchTask):
    ...
    def __init__(self, dee_setting, load_train=True, load_dev=True, load_test=True,
                 parallel_decorate=True, load_model=True):
        ...
        self.entity_label_list = DEEExample.get_entity_label_list()   # BIO labels of all entities
        # save_json(self.entity_label_list, 'entity_label_list')    #  five event types and their corresponding entities.

        self.event_type_fields_pairs = DEEExample.get_event_type_fields_pairs()
        # save_json(self.event_type_fields_pairs, 'event_type_fields_pairs')
        ...
        if load_model:
            if not self.setting.use_token_role:
                ...
                assert self.setting.model_type == 'CAB'
            ...
            if self.setting.model_type == 'CAB':
                self.model = CABModel(
    ...
    def resume_save_eval_at(self, epoch, train_eval=False, resume_cpt_flag=False, save_cpt_flag=True):
        ...
        if train_eval:     # coref
            eval_tasks = product(['train', 'dev', 'test'], [True, False])
        else:
            eval_tasks = product(['dev', 'test'], [True, False])
        ...
        for task_idx, (data_type, gold_span_flag) in enumerate(eval_tasks):
            ...
            elif data_type == 'train':         # coref
                features = self.train_features[:500]
                dataset = self.train_dataset[:500]
            else:
            ...
            self.eval(features, dataset, use_gold_span=gold_span_flag, dump_decode_pkl_name=decode_dump_name,
