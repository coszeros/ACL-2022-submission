...
from .utils import default_load_json, default_dump_json, default_dump_pkl, default_load_pkl, save_pkl
...
class DEEExample(object):
    def __init__(self, annguid, detail_align_dict, only_inference=False):
        ...
        if only_inference:
            ...
            # for coref
            self.span_abbr_pairs = detail_align_dict.get('span_abbr_pairs', [])
...
class DEEFeature(object):
    def __init__(self, guid, ex_idx, doc_token_id_mat, doc_token_mask_mat, doc_token_label_mat, entity_spans_list, span_idx2entity_idx,
        ...
        # coref
        # [[span_idx, ...], ...]  entity_idx -> list of span_idxs
        self.entity_spans_list = entity_spans_list
        # dict{span_idx: entity_idx}
        self.span_idx2entity_idx = span_idx2entity_idx
    ...
    def generate_dag_info_for(self, pred_span_token_tup_list, pred_span_idx2entity_idx, return_miss=False):
        # coref
        ...
        for gold_span_idx, token_tup in enumerate(self.span_token_ids_list):
            if token_tup in token_tup2pred_span_idx:
                ...
                gold_ent_idx = self.span_idx2entity_idx[gold_span_idx]
                pred_ent_idx = pred_span_idx2entity_idx[pred_span_idx]
                gold_ent_idx2pred_ent_idx[gold_ent_idx] = pred_ent_idx
        ...
        for event_arg_idxs_objs in self.event_arg_idxs_objs_list:
            if event_arg_idxs_objs is None:
                ...
                for event_arg_idxs in event_arg_idxs_objs:
                    pred_event_arg_idxs = []
                    for gold_ent_idx in event_arg_idxs:
                        if gold_ent_idx in gold_ent_idx2pred_ent_idx:
                            pred_event_arg_idxs.append(
                                gold_ent_idx2pred_ent_idx[gold_ent_idx]
                            )
                        else:
    ...
    def build_dag_info(event_arg_idxs_objs_list):
        ...
        # save_pkl(event_idx2field_idx2pre_path2cur_span_idx_set, 'event_idx2field_idx2pre_path2cur_span_idx_set')
...
class DEEFeatureConverter(object):
    ...
    def convert_example_to_feature(self, ex_idx, dee_example, log_flag=False):
        ...
        entity_spans_list        # List[(span_idxs)]  
        span_idx2entity_idx      # Dict{span_idx: entity_idx}
        replace span_idx with entity_idx
        """

        entity_spans_list = []
        handled_span = []
        truncated_flag = False
        for span, abbr in dee_example.span_abbr_pairs:
            span_idx = mspan2span_idx.get(span, None)
            abbr_idx = mspan2span_idx.get(abbr, None)
            if span_idx is None or abbr_idx is None:
                # print('mspan truncated.')
                truncated_flag = True
                continue
            entity_spans_list.append([span_idx, abbr_idx])
            if span in handled_span or abbr in handled_span:
                print(f'ReprocessError: {dee_example.guid}.')
            else:
                handled_span.extend([span, abbr])
        for mspan in mspan2span_idx:
            if mspan not in handled_span:
                mspan_idx = mspan2span_idx[mspan]
                entity_spans_list.append([mspan_idx])

        span_idx2entity_idx = {}
        for ent_idx, ent_span_idxs in enumerate(entity_spans_list):
            for span_idx in ent_span_idxs:
                span_idx2entity_idx[span_idx] = ent_idx
        assert len(span_idx2entity_idx) == len(mspan2span_idx)  # should have the same length.

        # check
        if log_flag:
            filepath = './save/check_dee_feature.json'
            new_feature = dict(guid=dee_example.guid, mspan=mspan2span_idx, span_abbr_pairs=dee_example.span_abbr_pairs,
                               entity_spans_list=entity_spans_list, span_idx2entity_idx=span_idx2entity_idx,
                               truncated_flag=truncated_flag)
            if os.path.exists(filepath):
                check_dee_feature = default_load_json(filepath)
                check_dee_feature.append(new_feature)
            else:
                check_dee_feature = [new_feature]
            default_dump_json(check_dee_feature, filepath)
                ...
                for event_obj in event_objs:
                    ...
                    for field in event_fields:
                        ...
                        if arg_span is None or arg_span not in mspan2span_idx:
                            ...
                            arg_span_idx = span_idx2entity_idx[mspan2span_idx[arg_span]]  # coref
                            any_valid_flag = True
        ...
        dee_feature = DEEFeature(
            ...
            entity_spans_list, span_idx2entity_idx,  # coref
            span_token_ids_list, span_dranges_list, event_type_labels, event_arg_idxs_objs_list,
...
def measure_dee_prediction(event_type_fields_pairs, features, event_decode_results,
    ...
    for term in event_decode_results:
        ...
        pred_record_mat = [
            [
                [
                    [tuple(arg_tup) for arg_tup in arg_tups] if arg_tups is not None else None  # coref
                    for arg_tups in pred_record
        ...
        gold_record_mat = [
            [
                [
                    [tuple(doc_fea.span_token_ids_list[span_idx]) for span_idx in doc_fea.entity_spans_list[arg_idx]] if arg_idx is not None else None  # coref
                    for arg_idx in event_arg_idxs
                ] for event_arg_idxs in event_arg_idxs_objs
...
def print_total_eval_info(data_span_type2model_str2epoch_res_list,
    ...
    for model_str in model_strs:
        ...
        for tgt_event_res in target_eval_res[:-1]:
            ...
            p, r, f1 = (tgt_event_res[0][key] for key in [p_key, r_key, f_key])
        ...
        ap, ar, af1 = (g_avg_res[key] for key in [p_key, r_key, f_key])
        eval_str += align_temp.format(eval_temp.format(ap, ar, af1))
...
def print_single_vs_multi_performance(mstr_bepoch_list, base_dir, features,
    ...
    for model_str, _ in mstr_bepoch_list:
        ...
        for _, single_f1, multi_f1 in model_str2etype_sf1_mf1_list[model_str]:
            eval_str += align_temp.format(eval_temp.format(single_f1, multi_f1))
            sf1s.append(single_f1)
            mf1s.append(multi_f1)
        avg_sf1 = np.mean(sf1s[:-1])
        avg_mf1 = np.mean(mf1s[:-1])
        eval_str += align_temp.format(eval_temp.format(avg_sf1, avg_mf1))
        print(eval_str)
...
def print_ablation_study(mstr_bepoch_list, base_dir, base_mstr, other_mstrs,
    ...
    for base_event_res in base_eval[:-1]:
        ...
        eval_str += align_temp.format(eval_temp.format(base_f1))
        bf1s.append(base_f1)
    g_avg_bf1 = base_eval[-1][f1_key]
    eval_str += align_temp.format(eval_temp.format(g_avg_bf1))
    avg_bf1 = np.mean(bf1s)
    eval_str += align_temp.format(eval_temp.format(avg_bf1))
    print(eval_str)
    ...
    for model_str in other_mstrs:
        if model_str in model_str2eval_res:
            ...
            for base_event_res, cur_event_res in zip(base_eval[:-1], cur_eval[:-1]):
                ...
                f1_abs = abs(f1_diff)
                if f1_diff >= 0:
                    eval_str += align_temp.format(inc_temp.format(f1_abs))
                else:
                    eval_str += align_temp.format(dec_temp.format(f1_abs))

            g_avg_f1_diff = cur_eval[-1][f1_key] - base_eval[-1][f1_key]
            g_avg_f1_abs = abs(g_avg_f1_diff)
            if g_avg_f1_diff >= 0:
                eval_str += align_temp.format(inc_temp.format(g_avg_f1_abs))
            else:
                eval_str += align_temp.format(dec_temp.format(g_avg_f1_abs))

            avg_f1_diff = np.mean(f1ds)
            avg_f1_abs = abs(avg_f1_diff)
            if avg_f1_diff >= 0:
                eval_str += align_temp.format(inc_temp.format(avg_f1_abs))
            else:
                eval_str += align_temp.format(dec_temp.format(avg_f1_abs))
