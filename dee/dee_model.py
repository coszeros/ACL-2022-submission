...
def get_doc_span_info_list(doc_token_types_list, doc_fea_list, use_gold_span=False):
    ...
    for doc_token_types, doc_fea in zip(doc_token_types_list, doc_fea_list):
        ...
        if use_gold_span:
            ...
            entity_spans_list = doc_fea.entity_spans_list
            span_idx2entity_idx = doc_fea.span_idx2entity_idx
        else:
            span_token_tup_list, span_dranges_list, entity_spans_list, span_idx2entity_idx = \
                extract_doc_valid_span_info(doc_token_type_mat, doc_fea)

            if len(span_token_tup_list) == 0 or len(span_token_tup_list) > 20:
                ...
                span_token_tup_list = doc_fea.span_token_ids_list
                span_dranges_list = doc_fea.span_dranges_list
                entity_spans_list = doc_fea.entity_spans_list  # coref
                span_idx2entity_idx = doc_fea.span_idx2entity_idx
        ...
        event_dag_info, _, missed_sent_idx_list = doc_fea.generate_dag_info_for(span_token_tup_list,  span_idx2entity_idx, return_miss=True)

        # doc_span_info will incorporate all span-level information needed for the event extraction
        doc_span_info = DocSpanInfo(
            entity_spans_list,  # coref
...
class CABModel(nn.Module):
    ...
    def __init__(self, config, event_type_fields_pairs, ner_model=None):
        ...
        self.skip_ed = config.skip_ed
        self.hybrid_extract = config.hybrid_extract
        ...
        self.event_tables = nn.ModuleList([
            EventTable(event_type, field_types, config.hidden_size, self.skip_ed)
    ...
    def get_doc_span_sent_context(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info):
        ...
        if doc_mention_emb is None:
            ...
            if self.config.use_doc_enc:
                ...
                for entity_spans in doc_span_info.entity_spans_list:
                    span_embs = [span_context_list[span_idx] for span_idx in entity_spans]
                    multi_span_context = torch.stack(span_embs).squeeze(1)
                    entity_context = multi_span_context.max(dim=0, keepdim=True)[0]
                    entity_context_list.append(entity_context)
                assert entity_context_list[0].shape == span_context_list[0].shape
                assert len(entity_context_list) == len(doc_span_info.entity_spans_list)
                ...
                # coref
                assert len(doc_fea.span_idx2entity_idx) == len(span_context_list)
        ...
        return entity_context_list, doc_sent_context
    ...
    def get_field_cls_info(self, event_idx, field_idx, batch_span_emb,
        ...
        if train_flag:
            ...
            return field_cls_loss, batch_span_logp, span_pred_list
        else:
            span_pred_list = batch_span_logp.argmax(dim=-1).tolist()
    ...
    def get_field_mle_loss_list(self, doc_sent_context, batch_span_context,
                                event_idx, field_idx2pre_path2cur_span_idx_set, core_num):
        field_mle_loss_list = []
        ...
        for field_idx in range(num_fields):
            ...
            for prev_path, cur_span_idx_set in prev_path2cur_span_idx_set.items():  # may have multiple pre_paths
                ...
                # calculate loss
                cur_field_cls_loss, batch_span_logp, span_pred_list = self.get_field_cls_info(
                ...
                for span_idx in cur_span_idx_set:
                    ...
                    if self.config.use_path_mem:
                        if not (self.hybrid_extract and field_idx >= core_num):
                            cur_decode_context = torch.cat([prev_decode_context, span_context], dim=0)
                        else:
                            cur_decode_context = prev_decode_context
    ...
    def get_loss_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info):
        ...
        all_field_loss_list = []
        for event_idx, event_label in enumerate(doc_fea.event_type_labels):
            core_num = event_idx2core_num[event_idx]
            if event_label == 0:
                ...
                for field_idx in range(num_fields):
                    ...
                    cur_field_cls_loss, batch_span_logp, _ = self.get_field_cls_info(
                        event_idx, field_idx, batch_cand_emb,
                        batch_span_label=batch_span_label, train_flag=True
                    )
                ...
                field_loss_list = self.get_field_mle_loss_list(doc_sent_context, batch_span_context, event_idx,
                                                               field_idx2pre_path2cur_span_idx_set, core_num)
                all_field_loss_list += field_loss_list

        total_event_loss = event_cls_loss + sum(all_field_loss_list) if not self.skip_ed else sum(all_field_loss_list)
    ...
    # def get_event_loss(self, doc_event_loss_list, doc_span_info_list):
    #     batch_size = len(doc_span_info_list)
    #     loss_batch_avg = 1.0 / batch_size
    #     return loss_batch_avg * sum(doc_event_loss_list)
    ...
    def get_eval_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info):
        ...
        for event_idx, event_pred in enumerate(event_pred_list):
            ...
            for decode_path in last_field_paths:
                ...
                if not list(filter(None, decode_path)):
                    continue
                field_idx2token_tup = []
                for span_idx in decode_path:
                    if span_idx is None:
                        ...
                        span_idxs = doc_span_info.entity_spans_list[span_idx]  # coref
                        token_tup = [doc_span_info.span_token_tup_list[s_idx] for s_idx in span_idxs]

                    field_idx2token_tup.append(token_tup)
                obj_idx2field_idx2token_tup.append(field_idx2token_tup)
            if obj_idx2field_idx2token_tup:
    ...
    def get_local_context_info(self, doc_batch_dict, train_flag=False, use_gold_span=False):
        ...
        ner_token_ids = torch.cat(ner_token_ids, dim=0)   # (sum_sent_num, max_sent_num(128))
        ner_token_masks = torch.cat(ner_token_masks, dim=0)
        if need_label_flag:
            ner_token_labels = torch.cat(ner_token_labels, dim=0)

        # get ner output
        ner_token_emb, ner_loss, ner_token_preds = self.ner_model(     # ner_token_emb (sum_sent_num, sent_num, hidden_size)
        ...
        ner_sent_emb = self.get_batch_sent_emb(ner_token_emb, ner_token_masks, valid_sent_num_list) # (sum_sent_num. hidden_size)

        assert sum(valid_sent_num_list) == ner_token_emb.size(0) == ner_sent_emb.size(0)  # (sum_sent_num)
    ...
    def forward(self, doc_batch_dict, doc_features,
        ...
        # Using scheduled sampling to gradually transits'c to predicted entity spans
        ...
        if train_flag:
            ...
            # mix_loss = self.get_event_loss(doc_event_loss_list, doc_span_info_list)
            return mix_loss
        else:
...
class EventTable(nn.Module):
    def __init__(self, event_type, field_types, hidden_size, skip_ed):
    ...
    def forward(self, sent_context_emb=None, batch_span_emb=None, field_idx=None):
        ...
        if not self.skip_ed and sent_context_emb is not None:  # [num_spans+num_sents, hidden_size]
...
def extract_doc_valid_span_info(doc_token_type_mat, doc_fea):
    ...
    coref 
    """
    pred_tup2idx = {tup: idx for idx, tup in enumerate(span_token_tup_list)}
    gold_span_token_tup_list = doc_fea.span_token_ids_list
    gold_entity_spans_list = doc_fea.entity_spans_list

    pred_entity_spans_list = []
    handled_pred_idx = []
    for ent_idx, span_idxs in enumerate(gold_entity_spans_list):
        assert len(span_idxs) >= 1
        if len(span_idxs) == 1:
            continue
        entity_token_tups = [gold_span_token_tup_list[span_idx] for span_idx in span_idxs]
        contain_flag = True
        pred_entity = []
        for token_tup in entity_token_tups:
            pred_span_idx = pred_tup2idx.get(token_tup, None)
            if pred_span_idx is not None:
                pred_entity.append(pred_span_idx)
            else:
                contain_flag = False
                break
        if contain_flag:
            pred_entity_spans_list.append(pred_entity)
            handled_pred_idx.extend(pred_entity)
    for pred_idx in range(len(span_token_tup_list)):
        if pred_idx not in handled_pred_idx:
            pred_entity_spans_list.append([pred_idx])

    pred_span_idx2entity_idx = {}
    for ent_idx, span_idxs in enumerate(pred_entity_spans_list):
        for span_idx in span_idxs:
            pred_span_idx2entity_idx[span_idx] = ent_idx
    assert len(pred_span_idx2entity_idx) == len(span_token_tup_list)

    return span_token_tup_list, span_dranges_list, pred_entity_spans_list, pred_span_idx2entity_idx
