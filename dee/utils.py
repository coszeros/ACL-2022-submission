...
def save_json(data, name, savepath=None, log=True):
    savepath = savepath or SAVEPATH
    datapath = savepath + name + '.json'
    json.dump(data, open(datapath, 'w'))
    if log:
        print(f'Successfully save: {datapath}')

def save_pkl(data, name, savepath=None, log=True):
    savepath = savepath or SAVEPATH
    datapath = savepath + name + '.pkl'
